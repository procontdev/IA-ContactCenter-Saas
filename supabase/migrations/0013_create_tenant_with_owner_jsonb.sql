-- Migration: 0013_create_tenant_with_owner_jsonb
-- Description: Reemplaza create_tenant_with_owner a retorno JSONB para eliminar ambigüedad por OUT vars implícitas.

DROP FUNCTION IF EXISTS platform_core.create_tenant_with_owner(TEXT, TEXT);

CREATE FUNCTION platform_core.create_tenant_with_owner(
    p_name TEXT,
    p_slug TEXT
)
RETURNS JSONB
...
BEGIN
    ...
    IF v_tenant_id IS NULL THEN
        ...
        VALUES (
            ...
            v_slug,
            true,
            jsonb_build_object('source', '0013_create_tenant_with_owner_jsonb', 'owner_user_id', v_user_id)
        )
