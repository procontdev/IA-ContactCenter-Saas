-- Migration: 0027_billing_readiness_subscription_scaffolding_mvp
-- Description: scaffolding MVP de suscripción por tenant (sin cobro real/pasarela).

CREATE TABLE IF NOT EXISTS saas_control.tenant_subscriptions (
    ...
    provider TEXT NOT NULL DEFAULT 'internal_scaffold',
    provider_ref TEXT NULL,
    metadata JSONB NOT NULL DEFAULT '{}'::jsonb,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    CONSTRAINT tenant_subscriptions_plan_code_check CHECK (plan_code IN ('basic', 'pro', 'enterprise')),
    CONSTRAINT tenant_subscriptions_status_check CHECK (status IN ('trial', 'active', 'past_due', 'suspended', 'canceled'))
);

DROP TRIGGER IF EXISTS tr_tenant_subscriptions_updated_at ON saas_control.tenant_subscriptions;
CREATE TRIGGER tr_tenant_subscriptions_updated_at
BEFORE UPDATE ON saas_control.tenant_subscriptions
...
SELECT
    ...
    now() + interval '1 month' AS current_period_ends_at,
    jsonb_build_object('seed_source', '0027_billing_readiness_subscription_scaffolding_mvp') AS metadata
FROM platform_core.tenants AS t
LEFT JOIN platform_core.tenant_settings AS ts ON ts.tenant_id = t.id
...
REVOKE ALL ON TABLE saas_control.tenant_subscriptions FROM PUBLIC;
GRANT SELECT, INSERT, UPDATE ON TABLE saas_control.tenant_subscriptions TO service_role;
